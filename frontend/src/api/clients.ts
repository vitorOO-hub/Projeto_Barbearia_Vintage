...
export async function createClient(payload: { name: string; email?: string; notes?: string }): Promise<Client> {
  const { data } = await apiClient.post<Client>("/api/v1/clients", payload);
  return data;
}

export async function updateClient(id: string, payload: Partial<{ name: string; email: string; notes: string }>): Promise<Client> {
